...
def extract_review():
    ...
    for apk, app in app_files:
        ...
        for l_id, line in enumerate(lines):
            ...
            rate = (
                float(terms[0]) if re.match(r"\d*\.?\d+", terms[0]) else 2.0
            )  # 2.0 is the average rate
            timed_reviews[apk].append(
                {"review": review, "date": date, "rate": rate, "version": version}
            )
            num_docs += 1
            num_words += wc
            if l_id % 1000 == 0:
...
def build_AOLDA_input_version(timed_reviews=None):
    ...
    for apk, reviews in timed_reviews.items():
        ...
        for ver in sorted(
            version_dict.keys(), key=lambda s: [int(v) for v in s.split(".")]
        ):
        ...
        dict_input = list(
            itertools.chain.from_iterable(list(itertools.chain.from_iterable(input)))
        )
        ...
        for t_i, text_period in enumerate(input):
            ...
            text_period = list(
                itertools.chain.from_iterable(text_period)
            )  # sentence level to doc level
            row = []
            col = []
            value = []
            r_id = 0
            for k, text in enumerate(text_period):
                ...
                if not empty:
                    r_id += 1
            input_X.append(
                sparse.coo_matrix((value, (row, col)), shape=(r_id, len(dictionary)))
            )
        OLDA_input[apk] = (
            dictionary,
            input_X,
...
def OLDA_fit(OLDA_input, n_topics, win_size):
    ...
    for apk, item in OLDA_input.items():
        ...
        olda_model = OLDA(
            n_topics=n_topics, n_iter=1000, refresh=500, window_size=win_size
        )
        ...
        for t_i, phi in enumerate(phis[apk]):
            ...
            for i, topic_dist in enumerate(phi):
                topic_words = [
                    dictionary[w_id] for w_id in np.argsort(topic_dist)[:-10:-1]
                ]
...
def get_candidate_label_ids(dic, labels, rawinput):
    all_label_ids = [dic.token2id.get(l) for l in labels]
    label_ids = []
            ...
            for id, value in bow:
                if id in label_ids[t_i]:
                    label_sensi[label_ids[t_i].index(id)].append(
                        [rates[t_i][d_i], len(doc_input)]
                    )  # record the rate and length
...
def sim_topic_word(phi, label_id, count):
    ...
    c_l = np.array(
        [
            np.log(
                (count[label_id, w_id] + 1)
                / float((count[w_id] + 1) * (count[label_id] + 1))
            )
...
def topic_labeling(OLDA_input, apk_phis, phrases, mu, lam, theta, save=True):
    ...
    for apk, item in OLDA_input.items():
        ...
        rawinput_sent = list(
            itertools.chain.from_iterable(list(itertools.chain.from_iterable(rawinput)))
        )
        ...
        for t_i, phi in enumerate(phis):
            ...
            topic_label_scores = topic_labeling_(
                count[t_i],
                total_count[t_i],
                label_ids[t_i],
                sensi_label[t_i],
            ...
            topic_label_sent_score = topic_label_sent(
                dictionary, phi, rawinput_sent, sent_ids[t_i], sensi_sent[t_i], mu, lam
            )
            ...
            if save:
                ...
                for tp_i, label_scores in enumerate(topic_label_scores):
                    ...
                    for w_id in np.argsort(label_scores)[: -candidate_num - 1 : -1]:
                        fout_labels.write(
                            "%s\t%f\t"
                            % (dictionary[label_ids[t_i][w_id]], label_scores[w_id])
                        )
                    fout_labels.write("\n")
...
def rank_topic_label(count, total_count, phi, label_ids, mu=0.2):
    ...
    for ind, label_id in enumerate(label_ids):
        for w_id in range(len(phi[0])):
            c_label_m[ind, w_id] = (
                count.get((label_id, w_id))
                * total_count
                / float((count.get(w_id) + 1) * (count.get(label_id) + 1))
                if (label_id, w_id) in count
                else 1.0
            )
...
def topic_detect(
    ...
    for ind, label_id in enumerate(label_ids):
        for w_id in range(len(phi[0])):
            c_label_m[ind, w_id] = (
                count.get((label_id, w_id))
                * total_count
                / float((count.get(w_id) + 1) * (count.get(w_id) + 1))
                if (label_id, w_id) in count
                else 1.0
            )
            c_last_label_m[ind, w_id] = (
                last_count.get((label_id, w_id))
                * last_total_count
                / float((last_count.get(w_id) + 1) * (last_count.get(label_id) + 1))
                if (label_id, w_id) in last_count
                else 1.0
            )
    ...
    phi_m = (
        (1 + mu / E) * phi_e
        - theta * last_phi[emerging_index]
        - mu / E * np.sum(phi_e, 0)
    )
    # TOPIC DETECT: construct residuals
    residuals_m = (
        (1 + mu / E) * np.log(phi_e) * phi_e
        - theta * np.log(phi_last_e) * phi_last_e
        - mu / E * np.sum(np.log(phi_e) * phi_e, 0)
    )
    # TOPIC DETECT: compute labels
    emerging_label_scores = (
        np.dot(
            (1 + mu / E) * phi_e - mu / E * np.sum(phi_e, 0), np.transpose(c_label_m)
        )
        - theta * np.dot(last_phi[emerging_index], np.transpose(c_last_label_m))
        + lam * sensi_label
    )
    emerging_sent_scores = (
        np.dot(phi_m, np.transpose(np.log(sent_count)))
        - np.sum(residuals_m, 1, keepdims=True)
        + lam * sensi_sent
    )
...
def topic_label_sent(dic, phi, rawinput_sents, sent_ids, sensi, mu, lam):
    ...
    residual_2 = (1 + mu_div) * np.sum(
        phi_logphi, 1, keepdims=True
    )  # residual_2 is a n_topic*1
    # construct sentence count matrix
    sent_count = np.empty((len(sent_ids), len(phi[0])), dtype=float)
    for ind, s_id in enumerate(sent_ids):
        bow = dic.doc2bow(rawinput_sents[s_id])
    ...
    phi_sent = (
        np.dot(c_phi, np.transpose(np.log(sent_count)))
        + residual_1
        - residual_2
        + lam * sensi
    )
